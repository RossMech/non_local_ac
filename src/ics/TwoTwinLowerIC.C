#include "TwoTwinLowerIC.h"
#include "FEProblem.h"
#include "MooseMesh.h"
#include <math.h>

registerMooseObject("NonLocalACApp", TwoTwinLowerIC);

InputParameters
TwoTwinLowerIC::validParams()
{
...
TwoTwinLowerIC::TwoTwinLowerIC(const InputParameters & parameters)
...
Real
TwoTwinLowerIC::value(const Point & p)
{
  ...
  if ((_int_width > n_tol))
  {
    ...
    if ((std::abs(res) < _int_width) && (std::abs(delta_r) < _int_width))
    {
      if (res < 0.0)
      {
        if (delta_r < 0.0)
