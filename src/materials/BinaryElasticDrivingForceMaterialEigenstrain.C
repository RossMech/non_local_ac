...
BinaryElasticDrivingForceMaterialEigenstrain::validParams()
{
	...
	params.addRequiredParam<std::string>("base_name_alpha","Base name for phase alpha");
	params.addRequiredParam<std::string>("base_name_beta","Base name for phase beta");
	params.addRequiredParam<std::string>("base_name","Global base name for mechanical properties");
	params.addRequiredCoupledVar("eta","phase variable");
	return params;
}

BinaryElasticDrivingForceMaterialEigenstrain::BinaryElasticDrivingForceMaterialEigenstrain(const InputParameters & parameters)
	: Material(parameters),
...
void
BinaryElasticDrivingForceMaterialEigenstrain::computeQpProperties()
{
	// Cutoff parameters
